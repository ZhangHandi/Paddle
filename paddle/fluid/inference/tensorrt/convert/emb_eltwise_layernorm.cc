 ...
 public:
  void operator()(const framework::proto::OpDesc& op,
    ...
    if (flag_varseqlen) {
      ...
      //LOG(ERROR) << "max_seq type: " << static_cast<int>(engine_->GetITensor("max_seqlen_tensor")->getType());
      //LOG(ERROR) << "word id dims: " << engine_->GetITensor(word_id_name)->getDimensions().d[0];
      ...
      //for (int i=0; i<=3; i++) {
      //    LOG(ERROR) << "plugin input data: " << i << "dims d[0]" << plugin_inputs.data()[i]->getDimensions().d[0];
      //}
      ...
      if (engine_->with_interleaved()) {
        ...
        shuffler_embed->setName(
            ("Emb_eltwise_out_shuffler_transpose (Output: " +
             op_desc.Output("Out")[0] + ")")
                .c_str());
        engine_->SetITensor(std::string("qkv_plugin_mask"), plugin_layer->getOutput(1));
      } else {
        layer = plugin_layer;
